...
const modalFormularioBuscar = new Modal(
  document.getElementById("modalFormularioBuscar"),
  {
    backdrop: "static",
    keyboard: false,
  }
...
const formularioLlenarAsimilados = document.getElementById(
  "formularioEvaluadoAsimilados"
);
// const buttonguardarmod = document.getElementById("buttonguardarmod");
const buttonImprimir = document.getElementById("buttonImprimir");
...
const formularioBoletasIngresadas = document.getElementById(
  "formularioBoletasIngresadas"
);
const botonFormularioBuscar = document.getElementById("btnBuscar1");
const btnBuscar = document.getElementById("btnBuscar");
const containerTableIngresos = document.getElementById(
  "containerTableIngresos"
);
const btnCancelarEvaluacion2 = document.getElementById(
  "btnCancelarEvaluacion2"
);
...
const buscarModalFillEvaluacion = async (e) => {
  ...
  try {
    ...
    formularioLlenarAsimilados.datevaluado_evaluado.value =
      datosBatz.per_catalogo;
    ...
    formularioLlenarAsimilados.datevaluado_dependencia.value =
      datosBatz.org_dependencia;
    formularioLlenarAsimilados.puestoEvaluado.value = datosBatz.per_desc_empleo;
    formularioLlenarAsimilados.datevaluado_puesto.value =
      datosBatz.per_desc_empleo;
...
const FillEvaluacionEvaluador = async (e) => {
  ...
  try {
    ...
    nombreInput.value = datosBatz.nombres;
    // nombreInput.style.fontSize = '12px';
    formularioLlenarAsimilados.datevaluador_dependencia.value =
      datosBatz.dep_llave;
    formularioLlenarAsimilados.puestoEvaluador.value =
      datosBatz.per_desc_empleo;
    formularioLlenarAsimilados.datevaluador_puesto.value =
      datosBatz.per_desc_empleo;
    formularioLlenarAsimilados.datevaluador_arma.value = datosBatz.arm_codigo;
    const numero = datosBatz.t_puesto;
...
const cerrarModalEvaluacion = async () => {
  ...
  buttonCargar.classList.add("d-none");
...
const mostrarAlerta = () => {
  alert("¡Botón Imprimir clickeado!");
};

const mostrarAlerta2 = () => {
  const radiosTodos = document.querySelectorAll('input[type="radio"]');

  radiosTodos.forEach((radio) => {
    radio.disabled = false;
    buttonCargar.disabled = true;
    buttonCargar.style.display = "none";
    buttonImprimir.disabled = true;
    buttonImprimir.style.display = "none";
    btnCancelarEvaluacion2.disabled = true;
    btnCancelarEvaluacion2.style.display = "none";
  });
  formularioLlenarAsimilados.cuadro1.disabled = false;
  formularioLlenarAsimilados.cuadro2.disabled = false;
  formularioLlenarAsimilados.cuadro3.disabled = false;
  formularioLlenarAsimilados.cuadro4.disabled = false;
  formularioLlenarAsimilados.btnGuardar.style.display = "block";
};
...
function sumaSubtotal() {
  ...
  notaCantidadBatz.forEach(function (notasValuePregs) {
    ...
    if (formularioLlenarAsimilados.subtotal1.value === "")
      formularioLlenarAsimilados.subtotal1.value = 0;
    if (formularioLlenarAsimilados.subtotal2.value === "")
      formularioLlenarAsimilados.subtotal2.value = 0;
    if (formularioLlenarAsimilados.subtotal4.value === "")
      formularioLlenarAsimilados.subtotal4.value = 0;
    if (formularioLlenarAsimilados.subtotal5.value === "")
      formularioLlenarAsimilados.subtotal5.value = 0;
    if (formularioLlenarAsimilados.subtotal6.value === "")
      formularioLlenarAsimilados.subtotal6.value = 0;
  ...
  sumaTotal =
    parseInt(formularioLlenarAsimilados.subtotal1.value) +
    parseInt(formularioLlenarAsimilados.subtotal2.value) +
    parseInt(formularioLlenarAsimilados.subtotal4.value) +
    parseInt(formularioLlenarAsimilados.subtotal5.value) +
    parseInt(formularioLlenarAsimilados.subtotal6.value);

  formularioLlenarAsimilados.asim_total_concep.value = sumaTotal;
  formularioLlenarAsimilados.subtotalGeneral2.value = sumaTotal;
  if (formularioLlenarAsimilados.subtotalGeneral1.value === "")
    formularioLlenarAsimilados.subtotalGeneral1.value = 0;
  if (formularioLlenarAsimilados.subtotalGeneral2.value === "")
    formularioLlenarAsimilados.subtotalGeneral2.value = 0;
...
function unblockPromedio() {
  if (
    ...
    const suma =
      parseFloat(cuadro1.value) +
      parseFloat(cuadro2.value) +
      parseFloat(cuadro3.value) +
      parseFloat(cuadro4.value);
...
function sumatoria1() {
  ...
  const llenarSuma =
    parseFloat(perfilBiofisico) +
    parseFloat(demeritos) +
    parseFloat(arrestos) +
    parseFloat(condicionFisica);
...
const buscarBoletaLlenada = async (e) => {
  ...
  buttonCargar.classList.remove("d-none");
  //  formularioLlenarAsimilados.btnGuardar.style.display = "block";
  ...
  try {
    ...
    switch (codigo) {
      case 1:
        ...
        formularioLlenarAsimilados.datevaluado_grado.value =
          datos.gradoevaluado;
        ...
        formularioLlenarAsimilados.datevaluador_evaluador.disabled = true;
        formularioLlenarAsimilados.datevaluador_evaluador.value =
          datos.evaluador;
        formularioLlenarAsimilados.gradoEvaluador.value = datos.gradoe;
        formularioLlenarAsimilados.nombre_evaluador.value =
          datos.nombresevaluador;
        formularioLlenarAsimilados.datevaluador_dependencia.value =
          datos.dependenciaevaluador;
        formularioLlenarAsimilados.puestoEvaluador.value =
          datos.puestoevaluador;
        formularioLlenarAsimilados.datevaluador_puesto.value =
          datos.puestoevaluador;
        ...
        let radioBotones = document.querySelectorAll(
          'input[class="checkBiofisico"]'
        );

        let casillaParaHabilitar = Array.from(radioBotones).find(
          (radio) => radio.value === perfilBiofisico
        );
        ...
        let radioBotonesPafe = document.querySelectorAll(
          'input[class="radioCondicionFisica"]'
        );

        let casillaParaHabilitarPafe = Array.from(radioBotonesPafe).find(
          (radio) => radio.value === pafe
        );
        ...
        let radioBotonesDemeritos = document.querySelectorAll(
          'input[class="demeritos"]'
        );

        let casillaParaHabilitarDemeritos = Array.from(
          radioBotonesDemeritos
        ).find((radio) => radio.value === demeritos);
        ...
        let radioBotonesArrestos = document.querySelectorAll(
          'input[class="arrestos"]'
        );

        let casillaParaHabilitArarrestos = Array.from(
          radioBotonesArrestos
        ).find((radio) => radio.value === arrestos);
        ...
        notas.forEach((nota, indice) => {
          formularioLlenarAsimilados["pregunta" + (indice + 1)].value =
            nota.pre_descripcion;

          formularioLlenarAsimilados["nota" + (indice + 1)].value =
            nota.notapsim_cantidad;

          const notas = formularioLlenarAsimilados["nota" + (indice + 1)].value;

          let radioBotonesNotas = document.querySelectorAll(
            'input[class="pregunta' + (indice + 1) + '"]'
          );

          let casillaParaHabilitarNotas = Array.from(radioBotonesNotas).find(
            (radio) => radio.value === notas
          );
...
const guardar = async (evento) => {
  ...
  body.append(
    "datevaluador_tiempo",
    formularioLlenarAsimilados.datevaluador_tiempo.value
  );
  body.append(
    "datevaluador_arma",
    formularioLlenarAsimilados.datevaluador_arma.value
  );
  ...
  body.append(
    "asim_demeritos",
    formularioLlenarAsimilados.asim_demeritos.value
  );
  body.append("asim_arrestos", formularioLlenarAsimilados.asim_arrestos.value);
  body.append(
    "asim_total_salud",
    formularioLlenarAsimilados.asim_total_salud.value
  );
  body.append(
    "asim_total_concep",
    formularioLlenarAsimilados.asim_total_concep.value
  );
  ...
  body.append(
    "asim_cat_comte",
    formularioLlenarAsimilados.asim_cat_comte.value
  );
...
// buttonguardarmod.disabled = true;
buscar();
formularioLlenarAsimilados.asim_cat_comte.addEventListener(
  "change",
  async (e) => {
    FillCatComte();
  }
);
formularioLlenarAsimilados.asim_cat_g1.addEventListener("change", async (e) => {
  FillCatG1();
});
formularioLlenarAsimilados.datevaluador_evaluador.addEventListener(
  "change",
  async (e) => {
    FillEvaluacionEvaluador();
  }
);
...
buttonImprimir.addEventListener("click", mostrarAlerta);
buttonCargar.addEventListener("click", mostrarAlerta2);
